...
- **命令前缀自适应**：插件回复中的命令提示会自动跟随当前 `COMMAND_START` 配置。QQ 官方机器人通常配置为 `/`，此时提示会显示 `/电费` 而非 `#电费`。
- **Markdown 消息**：QQ 官方机器人下回复使用 markdown 消息渲染，帮助菜单中的命令可直接点击触发（插入输入框），并带快捷按钮；OneBot 等不支持 markdown 的协议自动降级为纯文本。
- **数据迁移**：从 OneBot 切换为 QQ 官方机器人后，用户 ID 由 QQ 号变为 openid，原账号与订阅不会自动关联。用户可发送 `#迁移旧数据 你的旧QQ号`：校园卡账号会迁到当前 openid，记录宿舍则**在旧 QQ 号保留一份副本**，两边协议都能继续查询（该命令仅迁移本人旧数据，请勿填写他人 QQ 号）。
...
| #电费 缴费 区域 楼栋 房间号 金额 | 群员 | 否 | 私聊/群聊 | 从校园卡余额为指定宿舍充值电费 |
...
| #电费 管理 读数 区域 楼栋 房间号 日期 [时间] 剩余电量 | 管理员 | 否 | 私聊/群聊 | 手动录入日界读数并重算前一日 |
| #电费 管理 缴费 区域 楼栋 房间号 日期 [时间] 金额 | 管理员 | 否 | 私聊/群聊 | 手动录入缴费并重算该日 |
...
先使用 `#电费 记录 三期 21 1001` 设置宿舍。每天会在上海时区 00:00 查询一次剩余电量，并结算前一自然日的耗电量和电费。同一宿舍被多个用户设置时仍只会查询一次。
...
发送 `#电费 统计 [天数]` 可查看指定数量已结束自然日的总耗电、日均、最高耗电日及准确性统计；省略时默认为 30，回复会按实际已记录天数显示。例如：`#电费 统计7天`。

机器人回复会遮蔽房间号，避免在群聊中泄露宿舍隐私。管理员可使用 `#电费 管理 读数/缴费 区域 楼栋 房间号 日期 [时间] 数值` 手动补录；时间可省略，默认 `00:00`。手动读数会作为该日期的日界快照并重算前一日，手动缴费会参与该日结算。

#### 电费充值

先私聊设置校园卡账号（`#设置校园卡账号 学号 密码`），再发送 `#电费 缴费 区域 楼栋 房间号 金额` 发起缴费，机器人会返回待确认提示；回复 `#电费 缴费 确认` 才从校园卡余额扣除，`#电费 缴费 取消` 放弃（5 分钟内有效）。无需额外密码，例如：

```
#电费 缴费 三期 21 1001 50
→ 即将为三期2*栋1001 充值 50.00 元，将从校园卡余额中扣除。
   回复 #电费 缴费 确认 完成支付，或 #电费 缴费 取消 放弃。（5 分钟内有效）
#电费 缴费 确认
→ 缴费成功：三期2*栋1001 充值 50.00 元，已从校园卡余额扣除。
```

单次缴费金额不超过 999 元（与学校充值页一致，仅整数）。缴费成功后，该笔订单号（billno）会记录归属宿舍：即使你的校园卡给其他宿舍缴费，日结同步流水时也会按订单号归入对应宿舍，不会虚增你自己绑定宿舍的耗电；仅在机器人内完成的缴费有此归属记录，网页等其他渠道的缴费仍按「一卡一房」假设归入绑定宿舍。
...
| #迁移旧数据 你的旧QQ号 | 群员 | 否 | 私聊/群聊 | 迁移旧协议账号到当前账号；记录宿舍两边保留 |
