...
def build_reply(bot: Bot, text: str) -> str | MessageSegment:
    """生成适配器对应的回复消息。

    QQ 官方机器人使用 markdown 消息渲染,其余适配器降级为纯文本。
    命令前缀会按当前 ``command_start`` 配置重写（如 ``/``）。
    """
    text = rewrite_command_prefix(text, _command_prefix())
    if getattr(bot, "type", "") == "QQ":
        from nonebot.adapters.qq import MessageSegment as QQMessageSegment

        return QQMessageSegment.markdown(text)
